...
function fetchExpenses(page = 1) {
    ...
    axios.get(`https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/expense?page=${page}&limit=${itemsPerPage}`, { headers: { 'authorization': token } })
        .then((res) => {
...
function deleteExpense(expenseId) {
    const token = localStorage.getItem('token');
    axios.delete(`https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/expense/${expenseId}`, { headers: { 'authorization': token } })
        .then((res) => {
            fetchExpenses(currentPage);
        })
        .catch(err => {
...
//   console.log(token); 
  axios
    .get("https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/purchase/premiumMembership", {
    ...
    .then((response) => {
      var options = {
        ...
        handler: function (response) {
          axios
            .post(
              "https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/purchase/updateTransactionStatus",
                {
...
function checkPremiumStatus() {
    const token = localStorage.getItem('token');
    axios.get('https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/user/premiumStatus', { headers: { 'authorization': token } })
        .then((res) => {
...
function download() {
    const token = localStorage.getItem('token');
    axios.get('https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/user/download', { headers: { 'authorization': token } })
        .then((res) => {
            if (res.status === 200) {
...
function showLeaderboard(){  
    const token = localStorage.getItem('token');
    axios.get('https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/premium/leaderboard', { headers: { 'authorization': token } })
        .then((res) => {
...
function fetchDownloadHistory() {
    const token = localStorage.getItem('token');
    axios.get('https://expense-tracker-using-mongoose-xt7ie0020.vercel.app/download/history', { headers: { 'authorization': token } })
        .then((res) => {
